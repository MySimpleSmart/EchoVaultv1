  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.token) {
        ...
        // Fetch user profile to get email - try multiple endpoints
        try {
          // Start with any email provided by the JWT token response
          let userEmail = data.user_email || data.email || '';

          // Fallback 0: try decoding the JWT itself for an email claim
          try {
          ...
          // Try JWT token validation endpoint first
          ...
          // If no email from token validation, try users/me endpoint
          if (!userEmail) {
          ...
          // Fallback: store without email
          const userData = { username: username, user_email: '', email: '' };
          localStorage.setItem('user', JSON.stringify(userData));
          onLoginSuccess(data.token, userData);
        }
      } else {
