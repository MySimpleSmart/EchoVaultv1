          ...
          <div className="overflow-x-auto">
            ...
            <thead className="bg-gray-50">
              <tr>
                  ...
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Paid Total Payment</th>
              ...
              {schedule.map((r, index) => {
                    ...
                    // Calculate scheduled total payment (matching admin side logic)
                  ...
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 text-right font-medium">
                      {getCurrencySymbol(currency)}{scheduledTotalPayment.toFixed(2)}
                    </td>
                    <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 text-right">
                    ...
                    <td className="px-3 py-2 text-xs text-gray-600">{r.repayment_note || '-'}</td>
                </tr>
                    );
              })}
            </tbody>
            <tfoot className="bg-gray-100 border-t-2 border-gray-300 font-semibold">
              <tr>
                <td className="px-3 py-2 text-center" colSpan="5">TOTAL</td>
                <td className="px-3 py-2 text-right">{getCurrencySymbol(currency)}{(schedule[0]?.start_balance || 0).toFixed(2)}</td>
                <td className="px-3 py-2 text-right">{getCurrencySymbol(currency)}{schedule.reduce((sum, r) => sum + (parseFloat(r.accrued_interest) || 0), 0).toFixed(2)}</td>
                <td className="px-3 py-2 text-right">{getCurrencySymbol(currency)}{schedule.reduce((sum, r) => {
                  const scheduledPrincipal = r.scheduled_principal !== undefined 
                    ? (r.scheduled_principal || 0)
                    : ((r.start_balance || 0) - (r.remain_balance || 0));
                  const scheduledTotalPayment = r.scheduled_total_payment !== undefined
                ...
                <td className="px-3 py-2 text-right">{getCurrencySymbol(currency)}{schedule.reduce((sum, r) => sum + (parseFloat(r.paid_interest) || 0), 0).toFixed(2)}</td>
                <td className="px-3 py-2 text-right">{getCurrencySymbol(currency)}{schedule.reduce((sum, r) => sum + (parseFloat(r.paid_principles) || 0), 0).toFixed(2)}</td>
                <td className="px-3 py-2 text-right">{getCurrencySymbol(currency)}{schedule.reduce((sum, r) => sum + (parseFloat(r.total_payment) || 0), 0).toFixed(2)}</td>
                <td className="px-3 py-2 text-right">{getCurrencySymbol(currency)}{schedule.reduce((sum, r) => sum + (parseFloat(r.outstanding_interest) || 0), 0).toFixed(2)}</td>
                <td className="px-3 py-2 text-right">{getCurrencySymbol(currency)}{(schedule[schedule.length - 1]?.remain_balance || 0).toFixed(2)}</td>
        ...
        {products.map((p) => (
          ...
          >
            ...
            <div className="space-y-2 text-sm">
              ...
              {(p.term_min || p.term_max) && (
                <p className="text-gray-600">
                  <span className="font-medium">Term Range:</span>{' '}
                  {p.term_min && p.term_max
                    ? `${p.term_min} - ${p.term_max} months`
                    : p.term_min
                    ? `From ${p.term_min} months`
                    : p.term_max
                    ? `Up to ${p.term_max} months`
                    : ''}
                </p>
              )}
              {p.currency && (
                <p className="text-gray-600">
                  <span className="font-medium">Currency:</span> {p.currency}
                </p>
              )}
              {p.interest_rate && (
                <p className="text-gray-600">
